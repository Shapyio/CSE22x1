...
public final class SequencePalindrome {
    ...
    private static boolean isPalindrome(Sequence<Integer> s) {
        ...
        Sequence<Integer> sCopy = new Sequence1L<Integer>(); // New Sequence
        // sCopy copies s sequence
        for (int i = 0; i < s.length(); i++) {
            sCopy.add(i, s.entry(i));
        }
        sCopy.flip(); // Flips sCopy
        // Compares s and the flipped sCopy, returns true if Palindrome
        if (s.equals(sCopy)) {
            isPalin = true;
        }
