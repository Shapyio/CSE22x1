...
//        return false;
    ...
    public static <T> String treeToString(BinaryTree<T> t) {
        ...
        if (t.size() != 0) {
            T root = t.disassemble(lt, rt);
            str = root.toString() + "(" + treeToString(lt) + treeToString(rt)
                    + ")";
            t.assemble(root, lt, rt);
        } else {
            str = str + "()";
    ...
    public static BinaryTree<Integer> copy(BinaryTree<Integer> t) {
        BinaryTree<Integer> treeCopy = t.newInstance();
        BinaryTree<Integer> lt = t.newInstance();
        BinaryTree<Integer> rt = t.newInstance();

        if (t.height() > 0) {
            Integer root = t.disassemble(lt, rt);
            treeCopy.assemble(root, copy(lt), copy(rt));
            t.assemble(root, lt, rt);
        }
