...
public class Sequence3<T> extends SequenceSecondary<T> {
    ...
    private static <T> void setLengthOfLeftStack(Stack<T> leftStack,
        ...
        /*
         * If leftStack length is less than new given length, pop entries from
         * rightStack until left stack has the needed entries.
         */
        if (leftStack.length() < newLeftLength) {
            ...
            /*
             * If leftStack length is greater than required length, pop
             * leftStack entries until it meets required length
             */
        } else if (leftStack.length() > newLeftLength) {
            leftStack.flip(); // Reverse order mattered b/c of ensures clause
            while (leftStack.length() > newLeftLength) {
                leftStack.pop();
            }
        }
