...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (last >= left) {
            //If there is more than one child and right is smaller, make right smallest
            if (last > left && (order.compare(array[left], array[right]) < 0)) {
                smallest = right;
            }

            //If top > smallest: exchange and sift down
